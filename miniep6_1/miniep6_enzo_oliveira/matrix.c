...
void matrix_dgemm_2(int n, double *restrict _C, double *restrict _A, double *restrict _B)
{
    ...
    int i,j,k,linhaBloco,colunaBloco;
    int tamanhoBloco = 64;
    for (i = 0; i < n; i+=tamanhoBloco)
        for (k = 0; k < n; k+=tamanhoBloco)
        {
            for(linhaBloco = i ; linhaBloco < i +tamanhoBloco; linhaBloco++){
                 for(colunaBloco = k ; colunaBloco < k +tamanhoBloco; colunaBloco++){
                    for (j = 0; j < n; ++j){
                        C(linhaBloco,j) = C(linhaBloco,j) + (A(linhaBloco,colunaBloco)*B(colunaBloco,j));
